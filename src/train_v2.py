"""V2 training script: GISMo with nutrient-aware encoder.

Same data pipeline and training loop as v1 MVP, with two additions:

1. A per-node 7-dim (configurable) nutrient feature tensor is built and
   passed to the encoder, where it's projected through `nutrient_proj` and
   added to the learnable embedding before GIN propagation.

2. The 2-dim goal vector g (sugar, sodium) and L_health are kept exactly
   as in v1 MVP, so health loss is comparable.

REFACTORED for sparse ingredient ID support — see train_v1.py header for
details. Candidate pool is now `ingredient_ids` (sparse list of valid
ingredient node ids), not torch.arange(num_ingredients).

Outputs:
  - checkpoint:   best_v2.pt
  - predictions:  test_predictions_v2{_<g_label>}.json
"""
...
# Defaults for encoder nutrient injection. Order doesn't matter for v2
# (it's a learned linear projection). Keys missing from usda_mapping.json
# end up as a zero column after normalization — that's safe.
# NOTE: USDA JSON key is `calories_kcal` (plural), not `calorie_kcal`.
DEFAULT_ENCODER_NUTRIENT_KEYS = (
    "calories_kcal",
    "fat_g",
    "saturated_fat_g",
    "carbohydrate_g",
...
# ---------------------------------------------------------------------------
# Train / eval loops (nutrient_node threaded through encoder)
# ---------------------------------------------------------------------------

def train_one_epoch(model, loader, optimizer, edge_index, edge_weight,
                    nutrient_node, ingredient_ids, num_neg,
                    nutrient_health, lambda_h, margin, device):
    model.train()
    ...
    for batch in loader:
        ...
        h = model.encode_graph(edge_index, edge_weight, nutrient_node)
...
@torch.no_grad()
def evaluate(model, loader, edge_index, edge_weight, nutrient_node,
    ...
    for batch in loader:
        ...
        # Mask source (can't substitute itself). Convert raw id -> position.
...
def parse_args():
    ...
    # DEPRECATED — auto-detected from nodes_filtered.csv. Kept for backward
    # CLI compat (values are ignored).
    ...
    p.add_argument("--resume", type=str, default=None,
                   help="Explicit checkpoint path. If omitted, auto-resume from "
                        "<output_dir>/last_v2.pt when present.")
    p.add_argument("--no_resume", action="store_true",
                   help="Ignore any existing last_v2.pt and start fresh.")
    p.add_argument("--force_retrain", action="store_true",
                   help="Retrain even if best_v2.pt already exists (backs it "
    ...
    p.add_argument("--encoder_nutrient_keys", nargs="*",
                   default=list(DEFAULT_ENCODER_NUTRIENT_KEYS),
                   help="Nutrient keys (in usda_mapping.json) to inject "
                        "into the encoder. Default: 7 keys.")
...
def main():
    ...
    print(f"[config] mode=v2  device={device}")
    print(f"[config] encoder_nutrient_keys: {args.encoder_nutrient_keys}")
    ...
    # --- thresholds for g derivation (2-dim, sugar+sodium) ---
    ...
    print(f"[v2] tau_sugar={tau_sugar:.4f}  tau_sodium={tau_sodium:.4f}")
    ...
    # --- Nutrient tensors: both sized for num_total_nodes for direct id lookup. ---
    # (a) Per-node 7-d for encoder injection. Compound/gap rows are zero —
    #     fine since nutrient_proj has bias=False (zero in -> zero contribution).
    nutrient_node = load_nutrient_tensor(
        os.path.join(args.data_dir, "usda_mapping.json"),
        num_total_nodes=num_total_nodes,
        nutrient_keys=tuple(args.encoder_nutrient_keys),
    ).to(device)
    print(f"[v2] encoder nutrient tensor: shape {tuple(nutrient_node.shape)}")

    # Diagnostic: warn for nutrient columns with near-zero variance (key typo,
    # or key actually missing from the USDA JSON for all ingredients).
    populated = nutrient_node[ingredient_ids]
    col_std = populated.std(dim=0).cpu().numpy()
    for k_idx, k in enumerate(args.encoder_nutrient_keys):
        if col_std[k_idx] < 1e-4:
            print(f"[v2] WARNING: nutrient '{k}' has near-zero variance "
                  f"after norm — likely missing from usda_mapping.json")

    # (b) Per-node 2-d (sugar, sodium) for L_health.
    ...
    model = GISMo(
        ...
        nutrient_dim=len(args.encoder_nutrient_keys),
    ...
    best_ckpt_path = os.path.join(args.output_dir, "best_v2.pt")
    last_ckpt_path = os.path.join(args.output_dir, "last_v2.pt")
    ckpt_extra = {"num_total_nodes": num_total_nodes}

    start_epoch, best_mrr, epochs_no_improve = maybe_resume(
        args, "v2", args.output_dir, model, optimizer, device,
    )
    ...
    for epoch in range(start_epoch, args.max_epochs + 1):
        ...
        train_metrics = train_one_epoch(
            ...
            nutrient_node, ingredient_ids, args.num_neg,
            nutrient_health, args.lambda_h, args.margin, device,
        )
        val_metrics = evaluate(
            model, val_loader, edge_index, edge_weight, nutrient_node,
            ingredient_ids, ingredient_id_to_pos, device,
    ...
    cleanup_last_ckpt(args.output_dir, "v2")
    ...
    for label in args.test_g_overrides:
        ...
        suffix = f"_{label}"
        print(f"\n=== TEST [v2{suffix}] (best epoch {ckpt['epoch']}) ===")
        if override is not None:
            print(f"  g override: {override.tolist()}")
        test_metrics = evaluate(
            model, test_loader, edge_index, edge_weight, nutrient_node,
            ingredient_ids, ingredient_id_to_pos, device,
        ...
        pred_path = os.path.join(args.output_dir, f"test_predictions_v2{suffix}.json")
